...
FROM( SELECT
      ...
      WHERE [rb].[Meta__IsCurrent] = 1 ) AS [map]
...
CREATE OR ALTER VIEW [RCS_DW].[v_ReportLineBaseGroupMapping_GVAROL_MATERIALIZED]
AS
WITH [v_Cost_Center_Dim]
AS (
SELECT [CostCenter_SK], [CostCenter_AK], [CostCenterDesc] FROM [RCS_DW].[Cost_Center_Dim] WHERE [IsCurrent] = 1 )
SELECT
    [map].[ReportLineId]
  , [map].[ReportLineItem]
  , [map].[CostCenterDesc]
...
FROM( SELECT
        ...
        , [rbm].[CategoryDescNotInType]
      FROM [RCS_DW].[Report_Business_Mapping_Flattened_Rpt] AS [rb]
      INNER JOIN [v_Cost_Center_Dim] AS [cc] ON [rb].[CostCenter_SK] = [cc].[CostCenter_SK] OR [rb].[CostCenter_SK] = -9999
      INNER JOIN [RCS_DW].[v_Ledger_Account_Dim] AS [la] ON [la].[LedgerAccount_SK] = [rb].[LedgerAccount_SK]
      INNER JOIN [RCS_DW].[v_ReportLineMapping] AS [rbm] ON [rbm].[CategoryType] = [rb].[Category]
                                                        AND ( [rbm].[CostCenterType] IS NULL OR [rbm].[CostCenterType] = [rb].[CostCenterName] )
                                                        AND ( [rbm].[AccountType] IS NULL OR [rbm].[AccountType] = [la].[LedgerAccountName] )
